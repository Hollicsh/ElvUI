...
L["Adds an arrow in front of the chat lines to copy the entire line."] = "Добавляет стрелку в начале строк чата, позволяющую скопировать всю строку."
...
L["Aura Indicator (Class)"] = true
L["Aura Indicator (Global)"] = true
L["Aura Indicator (Pet)"] = true
L["Aura Indicator (Profile)"] = true
L["AuraBar Colors"] = true
L["Auras displayed on Blizzard's raid frames."] = true
L["Auras you can dispel."] = true
L["Auto Add New Spells"] = "Автоматически добавлять новые заклинания"
L["Auto Deposit Reagents"] = true
...
L["Checked: Holding Alt will focus arrow keys to the editbox.\nNot holding Alt will let you move the character even while editbox is open.\n\nUnchecked: Arrow keys always focused to editbox when open."] = true
...
L["Cooldown & Duration"] = "Восстановление и длительность"
...
L["Defensives that are self cast."] = true
L["Defensives that can be cast on others."] = true
...
L["Display Recent Ally icon after names in chat (excludes whispers and emotes)."] = true
...
L["Fade out the tooltip when it disappears, instant otherwise. Cursor anchored tooltips are unaffected."] = "Эффект затухания подсказки при скрытии, мгновенно скрывается при отключении. Подсказки, привязанные к курсору, не затрагиваются."
...
L["List of spells that will show when using Blizzard's assisted highlighting."] = true
...
L["The charge cooldown text."] = true
...
L["The loss of control cooldown text."] = true
...
L["This option controls whether nameplates will follow the settings below.\n\n|cffff3333Note:|r This is an override to the Motion Type setting."] = true
L["This option controls whether nameplates will follow the visibility settings below.\n\n|cffff3333Note:|r This will be overridden by the Enemy Combat Toggle."] = true
L["This option controls whether nameplates will follow the visibility settings below.\n\n|cffff3333Note:|r This will be overridden by the Friendly Combat Toggle."] = true
...
L["This section will help reset specific settings back to default."] = "Эта секция поможет вернуть настройки конкретного модуля на значения по умолчанию."
...
L["Tooltip gets anchored to bags when they are shown."] = true
...
L["Classification Color"] = true
L["Use classification color only in instances."] = true
...
L["Will attempt to sell another item in set interval after the previous one was sold."] = "Будет пытаться продать следующий предмет через установленный интервал после предыдущего."
L["Will display mana when the main power is:"] = "Будет отображать ману, если основной ресурс:"
...
L["Zero will inherit the unitframe general Multiplier. Disabling this override."] = true
...
L["|cffFF3333Note:|r Click the arrow on the dropdown box to see a list of spells."] = "|cffFF3333Внимание:|r Нажмите на стрелку у выпадающего списка ниже для просмотра списка заклинаний."
...
L["DEBUG_DESC"] = "Отключить все аддоны (включая плагины), кроме ElvUI. В рамках текущей сессии эту кнопку можно нажать, чтобы снова включить их."
L["DEFAULT"] = "По умолчанию"
L["DEMONIC_FURY"] = "Демоническая ярость"
L["DRESSUP_FRAME"] = "Примерочная"
L["DUNGEON_DIFFICULTY"] = "Сложность подземелья"
L["ELVUI_CREDITS"] = "Мы хотели бы выразить особую благодарность следующим людям за помощь в поддержке этого аддона - за тестирование, написание кода, а также тем, кто помогал нам через пожертвования."
...
L["FROST_ICICLES"] = "Кристаллы льда"
L["FURY"] = "Гнев"
...
L["OPACITY"] = "Видимость"
L["OPTION_TOOLTIP_ACTION_BUTTON_USE_KEY_DOWN"] = "Способности будут задействоваться, когда пользователь нажимает, а не отпускает клавишу.\n\n|cffff3333Примечание:|r Использование макроса с модификатором, совпадающим с клавишей действия подбора, принудительно переключит это на срабатывание по отпусканию клавиши."
...
----------------------------------
L["NoDuration"] = "Без длительности"
L["BlizzardNameplate"] = "Индикатор Blizzard"
L["Mount"] = "Транспорт"
L["CastByNPC"] = "Принадлежит NPC"
L["CastByPlayers"] = "Принадлежит игрокам"
...
L["TOGGLESKIN_DESC"] = "Вкл./выкл. этот скин."
...
--------- Available Tags ---------
L["[classcolor] but for the current target of the unit"] = "[classcolor], но для текущей цели цели"
L["Altpower"] = "Альтернативная энергия"
L["Changes color of the next tag based on how difficult the unit is compared to the players level"] = "Меняет цвет следующего тега в зависимости от сложности цели относительно уровня игрока"
L["Changes the color of the special power based upon its type"] = "Меняет цвет специальной энергии в зависимости от её типа"
L["Changes the text color to the current alternative power color (Blizzard defined)"] = "Меняет цвет текста на цвет текущей альтернативной энергии (определено Blizzard)"
L["Changes the text color, depending on the pet happiness"] = "Меняет цвет текста в зависимости от счастья питомца"
L["Changes the text color, depending on the unit's classification"] = "Меняет цвет текста в зависимости от классификации цели"
L["Changes the text color, depending on the unit's current health"] = "Меняет цвет текста в зависимости от текущего здоровья цели"
L["Changes the text color, depending on the unit's threat situation"] = "Меняет цвет текста в зависимости от угрозы цели"
L["Class"] = "Класс"
L["Class color name text, missing hp based on hex code"] = "Текст имени цветом класса, недостающее здоровье на основе hex-кода"
L["Class color name text, missing hp red"] = "Текст имени цветом класса, недостающее здоровье красным"
L["Classification"] = "Классификация"
L["Classpower"] = "Энергия класса"
L["Color your tag or text by replacing the XXXXXX with a hex color code. (Name tag as red text. Ex: ||cffFF0000[name]||r)"] = "Окрасьте тег или текст, заменив XXXXXX на hex-код цвета. (Тег имени красным текстом. Пример: ||cffFF0000[name]||r)"
L["Colors names by Faction (Alliance, Horde, Neutral)"] = "Окрашивает имена по фракции (Альянс, Орда, Нейтральные)"
L["Colors names by NPC reaction (Bad/Neutral/Good)"] = "Окрашивает имена по реакции NPC (Враждебные/Нейтральные/Дружественные)"
L["Colors names by player class or NPC reaction (Ex: [classcolor][name])"] = "Окрашивает имена по классу игрока или реакции NPC (Пример: [classcolor][name])"
L["Colors the following tags by difficulty, red for impossible, orange for hard, green for easy"] = "Окрашивает следующие теги по сложности: красный - невозможно, оранжевый - сложно, зелёный - легко"
L["Colors the power text based on the mana color"] = "Окрашивает текст энергии в цвет маны"
L["Colors the power text based upon its type"] = "Окрашивает текст энергии в зависимости от её типа"
L["Colors the text, depending on the type of the unit's selection"] = "Окрашивает текст в зависимости от типа выделения цели"
L["Deprecated version of [classcolor]"] = "Устаревшая версия [classcolor]"
L["Displays 'Alliance' or 'Horde'"] = "Отображает 'Альянс' или 'Орда'"
L["Displays 'L' if the unit is the group/raid leader"] = "Отображает 'L', если цель - лидер группы/рейда"
L["Displays 'Leader' if the unit is the group/raid leader"] = "Отображает 'Leader', если цель - лидер группы/рейда"
L["Displays 'OFFLINE' if the unit is disconnected"] = "Отображает 'OFFLINE', если цель не в сети"
L["Displays 'PvP' if the unit is pvp flagged"] = "Отображает 'PvP', если цель помечена для PvP"
L["Displays 'Rare' when the unit is a rare or rareelite"] = "Отображает 'Rare', если цель - редкий или редкий элитный монстр"
L["Displays 'zzz' if the unit is resting"] = "Отображает 'zzz', если цель отдыхает"
L["Displays <AFK> and <DND>"] = "Отображает <AFK> и <DND>"
L["Displays <AFK> if the unit is afk"] = "Отображает <AFK>, если цель не у компьютера"
L["Displays <DEAD> if the unit is dead"] = "Отображает <DEAD>, если цель мертва"
L["Displays a timer for how long a unit has had the status (e.g 'DEAD - 0:34')"] = "Отображает таймер, сколько времени цель имеет статус (например, 'DEAD - 0:34')"
L["Displays AFK/DND as an orange(afk) / red(dnd) icon"] = "Отображает AFK/DND в виде оранжевой (AFK) / красной (DND) иконки"
L["Displays all incoming heals"] = "Отображает все входящие исцеления"
L["Displays all players within 10 yards"] = "Отображает всех игроков в радиусе 10 ярдов"
L["Displays all players within 15 yards"] = "Отображает всех игроков в радиусе 15 ярдов"
L["Displays all players within 20 yards"] = "Отображает всех игроков в радиусе 20 ярдов"
L["Displays all players within 25 yards"] = "Отображает всех игроков в радиусе 25 ярдов"
L["Displays all players within 30 yards"] = "Отображает всех игроков в радиусе 30 ярдов"
L["Displays all players within 35 yards"] = "Отображает всех игроков в радиусе 35 ярдов"
L["Displays all players within 4 yards"] = "Отображает всех игроков в радиусе 4 ярдов"
L["Displays all players within 40 yards"] = "Отображает всех игроков в радиусе 40 ярдов"
L["Displays all players within 8 yards"] = "Отображает всех игроков в радиусе 8 ярдов"
L["Displays altpower text on a unit in current format"] = "Отображает текст альтернативной энергии цели в формате текущего значения"
L["Displays altpower text on a unit in current-max format"] = "Отображает текст альтернативной энергии цели в формате текущее/максимум"
L["Displays altpower text on a unit in current-max-percent format"] = "Отображает текст альтернативной энергии цели в формате текущее/максимум/процент"
L["Displays altpower text on a unit in current-percent format"] = "Отображает текст альтернативной энергии цели в формате текущее/процент"
L["Displays altpower text on a unit in deficit format"] = "Отображает текст альтернативной энергии цели в формате дефицита"
L["Displays altpower text on a unit in percent format"] = "Отображает текст альтернативной энергии цели в формате процента"
L["Displays amount of combo points the player has (only for player, shows nothing on 0)"] = "Отображает количество очков комбо у игрока (только для игрока; 0 - ничего не показывает)"
L["Displays current ElvUI users"] = "Отображает текущих пользователей ElvUI"
L["Displays honor level of the unit"] = "Отображает уровень чести цели"
L["Displays low level critter mobs"] = "Отображает низкоуровневых мобов"
L["Displays max HP without decimals"] = "Отображает максимальное HP без десятичных знаков"
L["Displays only incoming heals from other units"] = "Отображает только входящие исцеления от других целей"
L["Displays only personal incoming heals"] = "Отображает только личные входящие исцеления"
L["Displays percentage HP without decimals or the % sign. You can display the percent sign by adjusting the tag to [perhp<%]."] = "Отображает процент HP без десятичных знаков и знака %. Вы можете отобразить знак %, изменив тег на [perhp<%]."
L["Displays player name and title"] = "Отображает имя и звание игрока"
L["Displays player pvp rank icon"] = "Отображает иконку PvP-ранга игрока"
L["Displays player pvp rank number"] = "Отображает номер PvP-ранга игрока"
L["Displays player pvp title"] = "Отображает PvP-звание игрока"
L["Displays player title"] = "Отображает звание игрока"
L["Displays remaining time on pvp-flagged status"] = "Отображает оставшееся время статуса PvP-метки"
L["Displays the 'Alliance' or 'Horde' texture"] = "Отображает текстуру 'Альянс' или 'Орда'"
L["Displays the amount of absorbs"] = "Отображает количество поглощений"
L["Displays the amount of heal absorbs"] = "Отображает количество поглощений лечения"
L["Displays the arcane charges (Mage)"] = "Отображает заряды тайной магии (Маг)"
L["Displays the area spec of an unit"] = "Отображает специализацию области цели"
L["Displays the arena number 1-5"] = "Отображает номер арены 1-5"
L["Displays the character '+' if the unit is an elite or rare-elite"] = "Отображает символ '+', если цель - элитный или редкий элитный монстр"
L["Displays the chi points (Monk)"] = "Отображает очки ци (Монах)"
L["Displays the class icon of the unit, if that unit is a player"] = "Отображает иконку класса цели, если цель - игрок"
L["Displays the class of the unit, if that unit is a player"] = "Отображает класс цели, если цель - игрок"
L["Displays the creature type of the unit"] = "Отображает тип существа цели"
L["Displays the current additional mana of the unit and % when not full"] = "Отображает текущую дополнительную ману цели и %, если не полная"
L["Displays the current and max additional mana of the unit, separated by a dash (% when not full)"] = "Отображает текущую и макс. дополнительную ману цели, разделённых тире (%, если не полная)"
L["Displays the current and max hp of the unit, separated by a dash (% when not full hp)"] = "Отображает текущее и макс. HP цели, разделённых тире (%, если HP не полное)"
L["Displays the current and max hp of the unit, separated by a dash (% when not full hp), without status"] = "Отображает текущее и макс. HP цели, разделённых тире (%, если HP не полное), без статуса"
L["Displays the current and max mana of the unit, separated by a dash (% when not full)"] = "Отображает текущую и макс. ману цели, разделённых тире (%, если не полная)"
L["Displays the current and max mana of the unit, separated by a dash (% when not full) if their role is set to healer"] = "Отображает текущую и макс. ману цели, разделённых тире (%, если не полная), если роль игрока - лекарь"
L["Displays the current and maximum health of the unit, separated by a dash"] = "Отображает текущее и максимальное здоровье цели, разделённых тире"
L["Displays the current and maximum health of the unit, separated by a dash, without status"] = "Отображает текущее и максимальное здоровье цели, разделённых тире, без статуса"
L["Displays the current health as a shortvalue and then the full name of the unit when at full health"] = "Отображает текущее здоровье кратким значением, а затем полное имя цели, если здоровье полное"
L["Displays the current health as a shortvalue and then the name of the unit (limited to 10 letters) when at full health"] = "Отображает текущее здоровье кратким значением, а затем имя цели (ограничено 10 буквами), если здоровье полное"
L["Displays the current health as a shortvalue and then the name of the unit (limited to 15 letters) when at full health"] = "Отображает текущее здоровье кратким значением, а затем имя цели (ограничено 15 буквами), если здоровье полное"
L["Displays the current health as a shortvalue and then the name of the unit (limited to 20 letters) when at full health"] = "Отображает текущее здоровье кратким значением, а затем имя цели (ограничено 20 буквами), если здоровье полное"
L["Displays the current health as a shortvalue and then the name of the unit (limited to 5 letters) when at full health"] = "Отображает текущее здоровье кратким значением, а затем имя цели (ограничено 5 буквами), если здоровье полное"
L["Displays the current health of the unit"] = "Отображает текущее здоровье цели"
L["Displays the current health of the unit as a percentage"] = "Отображает текущее здоровье цели в процентах"
L["Displays the current health of the unit, without status"] = "Отображает текущее здоровье цели, без статуса"
L["Displays the current hp of the unit (% when not full hp)"] = "Отображает текущее HP цели (%, если HP не полное)"
L["Displays the current hp of the unit (% when not full hp), without status"] = "Отображает текущее HP цели (%, если HP не полное), без статуса"
L["Displays the current HP without decimals"] = "Отображает текущее HP без десятичных знаков"
L["Displays the current mana of the unit and % when not full"] = "Отображает текущую ману цели и %, если не полная"
L["Displays the current mana of the unit and % when not full if their role is set to healer"] = "Отображает текущую ману цели и % ,если не полная, если роль игрока - лекарь"
L["Displays the current power and max power, separated by a dash"] = "Отображает текущий и макс. ресурс, разделённых тире"
L["Displays the current power and max power, separated by a dash (% when not full power)"] = "Отображает текущий и макс. ресурс, разделённых тире (%, если ресурс не полный)"
L["Displays the current power and max power, separated by a dash (% when not full power) if their role is set to healer"] = "Отображает текущий и макс. ресурс, разделённых тире (%, если ресурс не полный), если роль игрока - лекарь"
L["Displays the current power and max power, separated by a dash if their role is set to healer"] = "Отображает текущий и макс. ресурс, разделённых тире, если роль игрока - лекарь"
L["Displays the current power and power as a percentage, separated by a dash"] = "Отображает текущий ресурс и процент ресурса, разделённых тире"
L["Displays the current power and power as a percentage, separated by a dash if their role is set to healer"] = "Отображает текущий ресурс и процент ресурса, разделённых тире, если роль игрока - лекарь"
L["Displays the current target of the unit"] = "Отображает текущую цель цели"
L["Displays the current target of the unit (limited to 10 letters)"] = "Отображает текущую цель цели (ограничено 10 буквами)"
L["Displays the current target of the unit (limited to 15 letters)"] = "Отображает текущую цель цели (ограничено 15 буквами)"
L["Displays the current target of the unit (limited to 20 letters)"] = "Отображает текущую цель цели (ограничено 20 буквами)"
L["Displays the current target of the unit (limited to 5 letters)"] = "Отображает текущую цель цели (ограничено 5 буквами)"
L["Displays the current target of the unit with transliteration for cyrillic letters"] = "Отображает текущую цель цели с транслитерацией кириллических букв"
L["Displays the current target of the unit with transliteration for cyrillic letters (limited to 10 letters)"] = "Отображает текущую цель цели с транслитерацией кириллических букв (ограничено 10 буквами)"
L["Displays the current target of the unit with transliteration for cyrillic letters (limited to 15 letters)"] = "Отображает текущую цель цели с транслитерацией кириллических букв (ограничено 15 буквами)"
L["Displays the current target of the unit with transliteration for cyrillic letters (limited to 20 letters)"] = "Отображает текущую цель цели с транслитерацией кириллических букв (ограничено 20 буквами)"
L["Displays the current target of the unit with transliteration for cyrillic letters (limited to 5 letters)"] = "Отображает текущую цель цели с транслитерацией кириллических букв (ограничено 5 буквами)"
L["Displays the current threat as a percent"] = "Отображает текущую угрозу в процентах"
L["Displays the current threat as a value"] = "Отображает текущую угрозу в виде значения"
L["Displays the current threat of lead as a percent"] = "Отображает текущую угрозу лидера в процентах"
L["Displays the current threat situation (Aggro is secure tanking, -- is losing threat and ++ is gaining threat)"] = "Отображает текущую ситуацию с угрозой (Aggro - надёжный танк, -- - потеря аггро, ++ - набор аггро)"
L["Displays the diet of your pet (Fish, Meat, ...)"] = "Отображает рацион Вашего питомца (Рыба, Мясо и т.д...)"
L["Displays the distance"] = "Отображает дистанцию"
L["Displays the first word of the unit's name"] = "Отображает первое слово имени цели"
L["Displays the full name of the unit without any letter limitation"] = "Отображает полное имя цели без ограничений по буквам"
L["Displays the group number the unit is in (1-8)"] = "Отображает номер группы, в которой находится цель (1-8)"
L["Displays the group number the unit is in (1-8): Only while in a raid."] = "Отображает номер группы, в которой находится цель (1-8): Только во время рейда."
L["Displays the guild name"] = "Отображает название гильдии"
L["Displays the guild name with < > and transliteration (e.g. <GUILD>)"] = "Отображает название гильдии с < > и транслитерацией (например, <GUILD>)"
L["Displays the guild name with < > brackets (e.g. <GUILD>)"] = "Отображает название гильдии в скобках < > (например, <GUILD>)"
L["Displays the guild name with transliteration for cyrillic letters"] = "Отображает название гильдии с транслитерацией кириллических букв"
L["Displays the guild rank"] = "Отображает ранг в гильдии"
L["Displays the health as a deficit and the name at full health"] = "Отображает здоровье как дефицит и имя при полном здоровье"
L["Displays the health deficit as a percentage and the full name of the unit"] = "Отображает дефицит здоровья в процентах и полное имя цели"
L["Displays the health deficit as a percentage and the name of the unit (limited to 10 letters)"] = "Отображает дефицит здоровья в процентах и имя цели (ограничено 10 буквами)"
L["Displays the health deficit as a percentage and the name of the unit (limited to 15 letters)"] = "Отображает дефицит здоровья в процентах и имя цели (ограничено 15 буквами)"
L["Displays the health deficit as a percentage and the name of the unit (limited to 20 letters)"] = "Отображает дефицит здоровья в процентах и имя цели (ограничено 20 буквами)"
L["Displays the health deficit as a percentage and the name of the unit (limited to 5 letters)"] = "Отображает дефицит здоровья в процентах и имя цели (ограничено 5 буквами)"
L["Displays the health deficit as a percentage, without status"] = "Отображает дефицит здоровья в процентах, без статуса"
L["Displays the health of the unit as a deficit (Total Health - Current Health = -Deficit)"] = "Отображает здоровье цели как дефицит (Макс. здоровье - Текущее здоровье = -Дефицит)"
L["Displays the health of the unit as a deficit, without status"] = "Отображает здоровье цели как дефицит, без статуса"
L["Displays the holy power (Paladin)"] = "Отображает силу Света (Паладин)"
L["Displays the last word of the unit's name"] = "Отображает последнее слово имени цели"
L["Displays the last word of the unit's target's name"] = "Отображает последнее слово имени цели цели"
L["Displays the level of the unit"] = "Отображает уровень цели"
L["Displays the max amount of mana the unit can have"] = "Отображает максимальное количество маны цели"
L["Displays the max amount of power of the unit in whole numbers without decimals"] = "Отображает максимальное количество ресурса цели целыми числами без десятичных знаков"
L["Displays the maximum health of the unit"] = "Отображает максимальное здоровье цели"
L["Displays the missing health of the unit in whole numbers, when not at full health"] = "Отображает недостающее здоровье цели целыми числами, если здоровье не полное"
L["Displays the missing power of the unit in whole numbers when not at full power"] = "Отображает недостающий ресурс цели целыми числами, если ресурс не полный"
L["Displays the name of the unit (limited to 10 letters)"] = "Отображает имя цели (ограничено 10 буквами)"
L["Displays the name of the unit (limited to 15 letters)"] = "Отображает имя цели (ограничено 15 буквами)"
L["Displays the name of the unit (limited to 20 letters)"] = "Отображает имя цели (ограничено 20 буквами)"
L["Displays the name of the unit (limited to 5 letters)"] = "Отображает имя цели (ограничено 5 буквами)"
L["Displays the name of the unit with abbreviation (e.g. 'Shadowfury Witch Doctor' becomes 'S. W. Doctor')"] = "Отображает имя цели с сокращением (например, 'Shadowfury Witch Doctor' становится 'S. W. Doctor')"
L["Displays the name of the unit with abbreviation (limited to 10 letters)"] = "Отображает имя цели с сокращением (ограничено 10 буквами)"
L["Displays the name of the unit with abbreviation (limited to 15 letters)"] = "Отображает имя цели с сокращением (ограничено 15 буквами)"
L["Displays the name of the unit with abbreviation (limited to 20 letters)"] = "Отображает имя цели с сокращением (ограничено 20 буквами)"
L["Displays the name of the unit with abbreviation (limited to 5 letters)"] = "Отображает имя цели с сокращением (ограничено 5 буквами)"
L["Displays the name of the unit with transliteration for cyrillic letters (limited to 10 letters)"] = "Отображает имя цели с транслитерацией кириллических букв (ограничено 10 буквами)"
L["Displays the name of the unit with transliteration for cyrillic letters (limited to 15 letters)"] = "Отображает имя цели с транслитерацией кириллических букв (ограничено 15 буквами)"
L["Displays the name of the unit with transliteration for cyrillic letters (limited to 20 letters)"] = "Отображает имя цели с транслитерацией кириллических букв (ограничено 20 буквами)"
L["Displays the name of the unit with transliteration for cyrillic letters (limited to 5 letters)"] = "Отображает имя цели с транслитерацией кириллических букв (ограничено 5 буквами)"
L["Displays the name of the unit's target with abbreviation (e.g. 'Shadowfury Witch Doctor' becomes 'S. W. Doctor')"] = "Отображает имя цели цели с сокращением (например, 'Shadowfury Witch Doctor' становится 'S. W. Doctor')"
L["Displays the name of the unit's target with abbreviation (limited to 10 letters)"] = "Отображает имя цели цели с сокращением (ограничено 10 буквами)"
L["Displays the name of the unit's target with abbreviation (limited to 15 letters)"] = "Отображает имя цели цели с сокращением (ограничено 15 буквами)"
L["Displays the name of the unit's target with abbreviation (limited to 20 letters)"] = "Отображает имя цели цели с сокращением (ограничено 20 буквами)"
L["Displays the name of the unit's target with abbreviation (limited to 5 letters)"] = "Отображает имя цели цели с сокращением (ограничено 5 буквами)"
L["Displays the NPC title (e.g. General Goods Vendor)"] = "Отображает звание NPC (например, Торговец общими товарами)"
L["Displays the NPC title with brackets (e.g. <General Goods Vendor>)"] = "Отображает звание NPC в скобках (например, <Торговец общими товарами>)"
L["Displays the percentage deficit health including absorb values. If greater than max health that will be reflected."] = "Отображает процентный дефицит здоровья с учётом поглощений. Если значение превышает максимальное здоровье, то это будет отражено."
L["Displays the pet happiness as a word (e.g. 'Happy')"] = "Отображает счастье питомца словом (например, 'Счастлив')"
L["Displays the pet happiness like a Discord emoji"] = "Отображает счастье питомца в виде эмодзи как в Discord"
L["Displays the pet happiness like the default Blizzard icon"] = "Отображает счастье питомца в виде стандартной иконки Blizzard"
L["Displays the pet loyalty level"] = "Отображает уровень верности питомца"
L["Displays the player's additional mana as a deficit"] = "Отображает дополнительную ману игрока как дефицит"
...
L["Displays the player's mana as a deficit"] = "Отображает ману игрока как дефицит"
L["Displays the player's mana as a deficit if their role is set to healer"] = "Отображает ману игрока как дефицит, если его роль - лекарь"
L["Displays the player's mana as a percentage"] = "Отображает ману игрока в процентах"
L["Displays the player's mana as a percentage if their role is set to healer"] = "Отображает ману игрока в процентах, если его роль - лекарь"
L["Displays the power as a deficit (Total Power - Current Power = -Deficit)"] = "Отображает ресурс как дефицит (Макс. ресурс - Текущий ресурс = -Дефицит)"
L["Displays the power as a deficit (Total Power - Current Power = -Deficit) if their role is set to healer"] = "Отображает ресурс как дефицит (Макс. ресурс - Текущий ресурс = -Дефицит), если роль игрока - лекарь"
L["Displays the quest count"] = "Отображает счётчик заданий"
L["Displays the quest objectives"] = "Отображает цели задания"
L["Displays the quest title"] = "Отображает название задания"
L["Displays the race"] = "Отображает расу"
L["Displays the runes (Death Knight)"] = "Отображает руны (Рыцарь смерти)"
L["Displays the server name"] = "Отображает название сервера"
L["Displays the server name with a dash in front (e.g. -Realm)"] = "Отображает название сервера с тире впереди (например, -Игровой мир)"
L["Displays the server name with transliteration for cyrillic letters"] = "Отображает название сервера с транслитерацией кириллических букв"
L["Displays the server name with transliteration for cyrillic letters and a dash in front"] = "Отображает название сервера с транслитерацией кириллических букв и тире впереди"
L["Displays the soulshards (Warlock)"] = "Отображает осколки души (Чернокнижник)"
L["Displays the specialization icon of the unit as text"] = "Отображает иконку специализации цели в виде текста"
L["Displays the specialization icon of the unit, if that unit is a player"] = "Отображает иконку специализации цели, если цель - игрок"
L["Displays the unit's classification (e.g. 'ELITE' and 'RARE')"] = "Отображает классификацию цели (например, 'ЭЛИТНЫЙ' и 'РЕДКИЙ')"
L["Displays the unit's classification in icon form (golden icon for 'ELITE' silver icon for 'RARE')"] = "Отображает классификацию цели в виде иконки (золотая иконка для 'ЭЛИТНОГО', серебряная для 'РЕДКОГО')"
L["Displays the unit's classification in short form (e.g. '+' for ELITE and 'R' for RARE)"] = "Отображает классификацию цели в краткой форме (например, '+' для ЭЛИТНОГО и 'R' для РЕДКОГО)"
L["Displays the unit's current additional mana"] = "Отображает текущую дополнительную ману цели"
L["Displays the unit's current amount of power"] = "Отображает текущее количество ресурса цели"
L["Displays the unit's current amount of power if their role is set to healer"] = "Отображает текущее количество ресурса цели, если роль игрока - лекарь"
L["Displays the unit's current amount of special power"] = "Отображает текущее количество специального ресурса цели"
L["Displays the unit's current amount of special power as a percentage"] = "Отображает текущее количество специального ресурса цели в процентах"
L["Displays the unit's current and max amount of special power, separated by a dash"] = "Отображает текущее и макс. количество специального ресурса цели, разделённых тире"
L["Displays the unit's current and max amount of special power, separated by a dash (% when not full power)"] = "Отображает текущее и макс. количество специального ресурса цели, разделённых тире (%, если ресурс не полный)"
L["Displays the unit's current and maximum additional mana, separated by a dash"] = "Отображает текущую и макс. дополнительную ману цели, разделённых тире"
L["Displays the unit's current and maximum mana, separated by a dash"] = "Отображает текущую и макс. ману цели, разделённых тире"
L["Displays the unit's current and maximum mana, separated by a dash if their role is set to healer"] = "Отображает текущую и макс. ману цели, разделённых тире, если роль игрока - лекарь"
L["Displays the unit's current and percentage amount of special power, separated by a dash"] = "Отображает текущее количество и процент специального ресурса цели, разделённых тире"
L["Displays the unit's current health as a percentage with absorb values"] = "Отображает текущее здоровье цели в процентах с учётом поглощений"
L["Displays the unit's current health as a percentage with absorb values, without status"] = "Отображает текущее здоровье цели в процентах с учётом поглощений, без статуса"
L["Displays the unit's current health as a percentage, without status"] = "Отображает текущее здоровье цели в процентах, без статуса"
L["Displays the unit's current mana"] = "Отображает текущую ману цели"
L["Displays the unit's current mana if their role is set to healer"] = "Отображает текущую ману цели, если роль игрока - лекарь"
L["Displays the unit's current power without decimals"] = "Отображает текущий ресурс цели без десятичных знаков"
L["Displays the unit's mana percentage without decimals"] = "Отображает процент маны цели без десятичных знаков"
L["Displays the unit's maximum power"] = "Отображает максимальный ресурс цели"
L["Displays the unit's percentage power without decimals"] = "Отображает процент ресурса цели без десятичных знаков"
L["Displays the unit's power as a percentage"] = "Отображает ресурс цели в процентах"
L["Displays the unit's power as a percentage if their role is set to healer"] = "Отображает ресурс цели в процентах, если роль игрока - лекарь"
L["Displays the unit's special power as a deficit (Total Special Power - Current Special Power = -Deficit)"] = "Отображает специальный ресурс цели как дефицит (Макс. спец. ресурс - Текущий спец. ресурс = -Дефицит)"
L["Displays zzz, dead, ghost, offline"] = "Отображает zzz, мертв, призрак, не в сети"
L["Guild"] = "Гильдия"
L["Health"] = "Здоровье"
L["Hunter"] = "Охотник"
L["Mana"] = "Мана"
L["Miscellaneous"] = "Разное"
L["Name text based on hex code, missing hp class color"] = "Текст имени на основе hex-кода, недостающее здоровье цветом класса"
L["Name text based on hex code, missing hp red"] = "Текст имени на основе hex-кода, недостающее здоровье красным"
L["Only display the unit's level if it is not the same as yours"] = "Показывать уровень цели только если он отличается от Вашего"
L["Party and Raid"] = "Группа и рейд"
L["Power"] = "Ресурс"
L["PvP"] = "PvP"
L["Quest"] = "Задание"
L["Quest full"] = "Задание выполнено"
L["Quest text"] = "Текст задания"
L["Range"] = "Радиус"
L["Realm"] = "Игровой мир"
L["Replace the name of the unit with 'DEAD' or 'OFFLINE' if applicable (limited to 10 letters)"] = "Заменить имя цели на 'МЕРТВ' или 'НЕ В СЕТИ', если применимо (ограничено 10 буквами)"
L["Replace the name of the unit with 'DEAD' or 'OFFLINE' if applicable (limited to 15 letters)"] = "Заменить имя цели на 'МЕРТВ' или 'НЕ В СЕТИ', если применимо (ограничено 15 буквами)"
L["Replace the name of the unit with 'DEAD' or 'OFFLINE' if applicable (limited to 20 letters)"] = "Заменить имя цели на 'МЕРТВ' или 'НЕ В СЕТИ', если применимо (ограничено 20 буквами)"
L["Replace the name of the unit with 'DEAD' or 'OFFLINE' if applicable (limited to 5 letters)"] = "Заменить имя цели на 'МЕРТВ' или 'НЕ В СЕТИ', если применимо (ограничено 5 буквами)"
L["Shortvalue of current and max hp (% when not full hp)"] = "Краткое значение текущего и макс. здоровья (%, если здоровье не полное)"
L["Shortvalue of current and max hp (% when not full hp, without status)"] = "Краткое значение текущего и макс. здоровья (%, если здоровье не полное, без статуса)"
L["Shortvalue of the current power and max power, separated by a dash"] = "Краткое значение текущего и макс. ресурса, разделённых тире"
L["Shortvalue of the current power and max power, separated by a dash (% when not full power)"] = "Краткое значение текущего и макс. ресурса, разделённых тире (%, если ресурс не полный)"
L["Shortvalue of the current power and max power, separated by a dash (% when not full power) if their role is set to healer"] = "Краткое значение текущего и макс. ресурса, разделённых тире (%, если ресурс не полный), если роль игрока - лекарь"
L["Shortvalue of the current power and max power, separated by a dash if their role is set to healer"] = "Краткое значение текущего и макс. ресурса, разделённых тире, если роль игрока - лекарь"
L["Shortvalue of the current power and power as a percentage, separated by a dash"] = "Краткое значение текущего ресурса и процента ресурса, разделённых тире"
L["Shortvalue of the current power and power as a percentage, separated by a dash if their role is set to healer"] = "Краткое значение текущего ресурса и процента ресурса, разделённых тире, если роль игрока - лекарь"
L["Shortvalue of the health deficit (e.g. -41k instead of -41300)"] = "Краткое значение дефицита здоровья (например, -41к вместо -41300)"
L["Shortvalue of the health deficit, without status"] = "Краткое значение дефицита здоровья, без статуса"
L["Shortvalue of the power as a deficit (Total Power - Current Power = -Deficit)"] = "Краткое значение дефицита ресурса (Макс. ресурс - Текущий ресурс = -Дефицит)"
L["Shortvalue of the power as a deficit (Total Power - Current Power = -Deficit) if their role is set to healer"] = "Краткое значение дефицита ресурса (Макс. ресурс - Текущий ресурс = -Дефицит), если роль игрока - лекарь"
L["Shortvalue of the unit's current amount of power (e.g. 4k instead of 4000)"] = "Краткое значение текущего ресурса цели (например, 4к вместо 4000)"
L["Shortvalue of the unit's current amount of power (e.g. 4k instead of 4000) if their role is set to healer"] = "Краткое значение текущего ресурса цели (например, 4к вместо 4000), если роль игрока - лекарь"
L["Shortvalue of the unit's current and max health, without status"] = "Краткое значение текущего и максимального здоровья цели, без статуса"
L["Shortvalue of the unit's current and max hp, separated by a dash"] = "Краткое значение текущего и максимального здоровья цели, разделённых тире"
L["Shortvalue of the unit's current health (e.g. 81k instead of 81200)"] = "Краткое значение текущего здоровья цели (например, 81к вместо 81200)"
L["Shortvalue of the unit's current health without status"] = "Краткое значение текущего здоровья цели, без статуса"
L["Shortvalue of the unit's current hp (% when not full hp)"] = "Краткое значение текущего здоровья цели (%, если здоровье не полное)"
L["Shortvalue of the unit's current hp (% when not full hp), without status"] = "Краткое значение текущего здоровья цели (%, если здоровье не полное), без статуса"
L["Shortvalue of the unit's maximum health"] = "Краткое значение максимального здоровья цели"
L["Shortvalue of the unit's maximum power"] = "Краткое значение максимального ресурса цели"
L["Shortvalue of the unit's maximum power if their role is set to healer"] = "Краткое значение максимального ресурса цели, если роль игрока - лекарь"
L["Speed"] = "Скорость передвижения"
L["Target"] = "Цель"
L["Threat"] = "Угроза"
L["White name text, missing hp red"] = "Белый текст имени, недостающее здоровье красным"
L["||cffXXXXXX [tag] or text here ||r"] = "||cffXXXXXX [тег] или текст здесь ||r"
----------------------------------
